...
for bond in bonds:
    ...
    if bond == (8, 20):
        ...
        sbn.distplot(pfizer_results[ser_bond]['wbo_dist'], hist=False, color=sbn.color_palette()[1])

        sbn.kdeplot(results['0.03_path_length_False_None']['wbo_dist'], shade=True)
        sbn.distplot(results['0.03_path_length_False_None']['wbo_dist'], rug=True, hist=False,
    ...
    plt.tight_layout()
    plt.savefig('wbo_dists_bond_{}_{}.pdf'.format(bond[0], bond[1]))

    colors = [rbg_to_int(list(i), alpha=255) for i in sbn.color_palette()]
    if bond == (8, 20):
        wbos = [results['parent']['elf10_wbo'],  pfizer_results[ser_bond]['elf10_wbo'], results['0.03_path_length_False_None']['elf10_wbo']]

        frags = [results['parent']['frag'], pfizer_results[ser_bond]['frag'], results['0.03_path_length_False_None']['frag']]
    else:
        wbos = [results['parent']['elf10_wbo'],
                pfizer_results[ser_bond]['elf10_wbo']]
        frags = [results['parent']['frag'],
                 pfizer_results[ser_bond]['frag']]
    visualize_mols(frags, cols=2, rows=2, bond_idx=bond, colors=colors, wbos=wbos, fname='fragments_bond_{}_{}.pdf'.format(bond[0], bond[1]), align_to=0)