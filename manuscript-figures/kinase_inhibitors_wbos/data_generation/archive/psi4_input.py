...
for mol in mollist:
    ...
    for i, coords in enumerate(xyz.split('*')):
        ...
        with open('geom_opt_bsub.lsf', 'r') as file:
            filedata = file.read()
        ...
        bsub_file = os.path.join(os.getcwd(), '{}/{}_{}_geom_opt.lfs'.format(name, name, str(i)))
        with open(bsub_file, 'w') as file:
            file.write(filedata)
        #change directory and submit job
        os.chdir(os.path.join(os.getcwd(), name))
        print(os.getcwd())
        stdin_file = open('{}_{}_geom_opt.lfs'.format(name, str(i)), 'r')
