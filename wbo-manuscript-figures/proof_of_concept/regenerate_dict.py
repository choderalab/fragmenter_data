...
for i, fgroup in enumerate(both_plots):
    ...
    with open("mappedsmiles2.pkl", "wb") as f:
        pickle.dump(smilesDict, f)
def makeCovPlot(filename):
    ...
    for key, data in plotdata.items():
        ...
        plt.plot(np.unique(am1_wbos), np.poly1d([slope, intercept])(np.unique(am1_wbos)), color_keys[count])
        plt.scatter(x=am1_wbos, y=max_energies, color=color_keys[count], marker=symbols[count], label=key)
        colors.append(color_keys[count])
        count+=1
    ...
    #plt.ylim(0, 100)
    plt.tight_layout()
    plt.savefig('energy_vs_wbo_full.pdf')
